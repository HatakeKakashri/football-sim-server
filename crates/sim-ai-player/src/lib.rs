...
pub fn perception_system(
    ...
    other_players: Query<(&Player, &Position)>,
) {
    // Get ball position
    let ball_position = if let Ok(ball_pos) = ball_query.get_single() {
        ball_pos.0
    } else {
        Vec2::zero()
    };
    
    // For each player, build perception snapshot
    for (mut player, player_pos) in query.iter_mut() {
        let mut nearby_teammates = smallvec::SmallVec::new();
        let mut nearby_opponents = smallvec::SmallVec::new();
        
        // Find nearby players
        for (other_player, other_pos) in other_players.iter() {
            let distance = player_pos.0.distance(other_pos.0);
            
            // Only consider players within 20 meters
            if distance <= 20.0 {
                let relative_position = other_pos.0 - player_pos.0;
                let nearby_entity = sim_components::NearbyEntity {
                    entity: Entity::PLACEHOLDER, // We don't have the entity here
                    distance,
                    relative_position,
                };
        ...
        // Calculate pitch bounds (assuming standard 105m x 68m pitch)
        let pitch_bounds = sim_components::PitchBounds {
            distance_to_left: player_pos.0.x,
            distance_to_right: 105.0 - player_pos.0.x,
            distance_to_top: player_pos.0.y,
            distance_to_bottom: 68.0 - player_pos.0.y,
        };
        
        // Calculate goal position (assuming attacking right goal)
...
pub fn player_action_execution_system(mut query: Query<(&Player, &mut Velocity)>) {
    ...
    for (player, mut velocity) in query.iter_mut() {
        if let Some(intent) = &player.intent {
            match intent {
                sim_components::Intent::MoveTo(target) => {
                ...
                sim_components::Intent::Shoot(_target) => {
                    // For now, just stop
                    velocity.0 = Vec2::zero();
                }
                _ => {
...
mod tests {
    ...
    fn test_stamina_based_decision() {
        ...
        let utility_brain = UtilityBrain {
            actions: vec![PlayerAction {
                intent: sim_components::Intent::MoveTo(Vec2::new(100.0, 34.0)),
                considerations: vec![],
            }],
            hysteresis: 0.1,
    ...
    fn test_defender_tackle() {
        ...
        world.entity_mut(attacker_entity).insert(Player {
            ...
            role: Role::Striker,
            skill: 0.8,
            intent: Some(sim_components::Intent::Shoot(Vec2::new(105.0, 34.0))),
            perception: None,
        });
